...
# -*- coding: utf-8 -*-
"""
run_weekly_lock_tuning.py
주간 최대 낙폭(Weekly MDD Lock) 한도를 조정하여 고수익/MDD ~26% 타겟 튜닝
"""
import asyncio
...
def simulate(data, btc1h_map, weekly_mdd_limit):
    cash = INITIAL_EQUITY
    positions = {}
    trades = []
    fees = 0.0
    ...
    for idx, t in enumerate(all_ts):
        ...
        if weekly_mdd_limit is not None and weekly_mdd_pct <= weekly_mdd_limit:
            weekly_locked = True
        ...
        for sym, df in data.items():
            ...
            # Falling knife limit is disabled for this test to allow high yield
...
async def main():
    ...
    try:
        ...
        print(f"\n[MDD 26% 타겟] 주간 하드락 한도(Weekly MDD Lock) 완화 테스트\n")
        limits = [
            ("주간락 -10% (현재)", -10.0),
            ("주간락 -20% (완화)", -20.0),
            ("주간락 -25% (완화)", -25.0),
        ...
        hdr = f"{'주간락기준':18s} {'전체수익':>10s} {'E1횡보':>10s} {'E2랠리':>10s} {'E3최근폭락':>10s} {'MDD%':>6s}"
        print(hdr)
        print("─" * len(hdr))
        for name, lim in limits:
            r = simulate(data, btc_map, weekly_mdd_limit=lim)
            e = r['eras']
