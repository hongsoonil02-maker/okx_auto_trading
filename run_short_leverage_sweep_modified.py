...
# -*- coding: utf-8 -*-
"""
run_short_leverage_sweep_modified.py
숏 이중 게이트(EMA50 하회) 로직 하에서 숏 레버리지(5x, 10x, 20x) 스윕 백테스트
"""
import asyncio
...
async def fetch_all(ex, sym, tf, since_ms):
    ...
    since = since_ms
    for _ in range(40):
        print(f"fetching {sym} {tf} since {since}..."); batch = await ex.fetch_ohlcv(sym, tf, since=since, limit=300)
        if not batch: break
        out.extend(batch)
...
def simulate(data, btc_map, short_leverage, use_short_gate=False):
    cash = INITIAL_EQUITY
    positions = {}
    trades = []
    fees = 0.0
    ...
    for idx, t in enumerate(all_ts):
        ...
        for p in positions.values():
            lev = LONG_LEVERAGE if p['dir'] == 1 else short_leverage
            equity += p['margin'] * (1 + (p['last_px'] - p['entry']) / p['entry'] * lev * p['dir'])
        
        eq_peak = max(eq_peak, equity)
        dd = (eq_peak - equity) / eq_peak * 100 if eq_peak > 0 else 0
        mdd = max(mdd, dd)
        ...
        is_cb_active = cb_tripped or (t < cb_cooldown_until)

        last_h = ((t // 3600000) * 3600000) - 3600000
        _m = btc_map.get(last_h, (20.0, True, 0.0, True))
        adx_now, btc_bull, btc_dd, btc_above_ema50 = _m[0], _m[1], _m[2], _m[3]
        ...
        for sym, df in data.items():
            ...
            if pos:
                lev = LONG_LEVERAGE if pos['dir'] == 1 else short_leverage
                pos['last_px'] = px
            ...
            # Short Dual Gate Logic (EMA50)
            if short_sig and use_short_gate:
                if curr['c'] >= curr['ema50'] or btc_above_ema50:
                    short_sig = False
            ...
            lev = LONG_LEVERAGE if d == 1 else short_leverage

            equity = cash
            for p in positions.values():
                elev = LONG_LEVERAGE if p['dir'] == 1 else short_leverage
...
async def main():
    ...
    try:
        ...
        for i in range(5, len(btc) - 1):
            ...
            above50 = bool(r['c'] > r['ema50'])
            btc_map[int(r['t'])] = (float(r['adx']), bullish, float(r['dd']), above50)
        ...
        print(f"\n[CTO Andre] 숏 이중 게이트(EMA50 하회 조건) 끄고 켰을 때의 레버리지 스윕\n")
        leverages = [5, 10, 20]
        
        for gate_on in [False, True]:
            gate_str = "[ON]" if gate_on else "[OFF]"
            print(f"--- 숏 이중 게이트 {gate_str} ---")
            hdr = f"{'숏레버리지':10s} {'전체수익':>10s} {'E1횡보':>10s} {'E2랠리':>10s} {'E3최근폭락':>10s} {'숏손익':>10s} {'MDD%':>6s}"
            print(hdr)
            print("─" * len(hdr))
            for lev in leverages:
                r = simulate(data, btc_map, short_leverage=lev, use_short_gate=gate_on)
                e = r['eras']
                print(f"  {str(lev)+'x':8s} {r['total']:+10.0f} {e['E1 횡보(6/23~7/24)']:+10.0f} {e['E2 랠리(7/25~8/20)']:+10.0f} "
