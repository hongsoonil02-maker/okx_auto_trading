...
# -*- coding: utf-8 -*-
"""
run_short_leverage_sweep.py
스마트 서킷 브레이커 로직 하에서 숏 레버리지(3x, 5x, 10x, 20x) 스윕 백테스트
"""
import asyncio
...
def simulate(data, btc1h_map, short_leverage):
    cash = INITIAL_EQUITY
    positions = {}
    trades = []
    fees = 0.0
...
async def main():
    ...
    try:
        ...
        print(f"\n[CTO Andre] 스마트 서킷 브레이커 하에서의 숏 레버리지 최적화 스윕\n")
        leverages = [3, 5, 10, 20]
        hdr = f"{'숏레버리지':10s} {'전체수익':>10s} {'E1횡보':>10s} {'E2랠리':>10s} {'E3최근폭락':>10s} {'숏손익':>10s} {'MDD%':>6s}"
        print(hdr)
        print("─" * len(hdr))
        for lev in leverages:
            r = simulate(data, btc_map, short_leverage=lev)
            e = r['eras']
