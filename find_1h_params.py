...
def simulate(df, ema_period=200, tight_mult=2.5, loose_mult=4.0, vol_mult=1.2, min_hold=3, max_dca=3):
    trades = []
    ...
    for i in range(250, len(df)):
        ...
        is_ema_up = curr['c'] > curr['ema_target']
        is_ema_down = curr['c'] < curr['ema_target']
...
async def main():
    syms = ["BTC/USDT:USDT", "ETH/USDT:USDT", "SOL/USDT:USDT", "TSLA/USDT:USDT", "AAPL/USDT:USDT", "NVDA/USDT:USDT"]
    ex = ccxt_async.okx({'enableRateLimit': True, 'options': {'defaultType': 'swap'}})
    
    print("Testing 1h settings on Majors (EMA Comparison)")
    print(f"{'Symbol':<15} | {'EMA 50 (Current)':<22} | {'EMA 20':<22} | {'EMA 10':<22}")
    try:
        for sym in syms:
            try:
                ...
                rsi = 100 - (100 / (1 + rs))
                sr = (rsi - rsi.rolling(14).min()) / (rsi.rolling(14).max() - rsi.rolling(14).min())
                df['stoch_k'] = sr.rolling(3).mean() * 100

                # EMA 50
                tr50 = simulate(df.copy(), ema_period=50, tight_mult=2.5, loose_mult=4.0, vol_mult=1.2)
                # EMA 20
                tr20 = simulate(df.copy(), ema_period=20, tight_mult=2.5, loose_mult=4.0, vol_mult=1.2)
                # EMA 10
                tr10 = simulate(df.copy(), ema_period=10, tight_mult=2.5, loose_mult=4.0, vol_mult=1.2)
                
                s50 = stats(tr50); s20 = stats(tr20); s10 = stats(tr10)
                f = lambda s: f"{s['ret']:>6.2f}% ({s['n']:>2d})"
                print(f"{sym:<15} | {f(s50):<22} | {f(s20):<22} | {f(s10):<22}")
