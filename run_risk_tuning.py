...
"""
run_risk_tuning.py
MDD ~26% 타겟 고수익 튜닝 백테스트 (하따 기준 조정)
"""
import asyncio
...
def simulate(data, btc1h_map, falling_knife_limit):
    cash = INITIAL_EQUITY
    positions = {}
    trades = []
    fees = 0.0
    ...
    for idx, t in enumerate(all_ts):
        ...
        for sym, df in data.items():
            ...
            # Falling knife check (-X% in last 24h)
            if i >= 96 and falling_knife_limit is not None:
                price_24h_ago = df.iloc[i-96]['c']
                if (px - price_24h_ago) / price_24h_ago <= falling_knife_limit:
                    allow_long = False
...
async def main():
    ...
    try:
        ...
        print(f"\n[MDD 26% 타겟] 칼날 잡기(하따) 기준 조정을 통한 고수익 튜닝\n")
        # None means no limit (full knife catching)
        limits = [
            ("현재(-10% 락)", -0.10),
            ("완화(-15% 락)", -0.15),
            ("완화(-20% 락)", -0.20),
        ...
        hdr = f"{'하따기준':16s} {'전체수익':>10s} {'E1횡보':>10s} {'E2랠리':>10s} {'E3최근폭락':>10s} {'MDD%':>6s}"
        print(hdr)
        print("─" * len(hdr))
        for name, lim in limits:
            r = simulate(data, btc_map, falling_knife_limit=lim)
            e = r['eras']
            print(f"  {name:14s} {r['total']:+10.0f} {e['E1 횡보(6/23~7/24)']:+10.0f} {e['E2 랠리(7/25~8/20)']:+10.0f} "
