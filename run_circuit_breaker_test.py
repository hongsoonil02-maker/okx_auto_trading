...
# -*- coding: utf-8 -*-
"""
run_circuit_breaker_test.py
서킷 브레이커 옵션 비교 백테스트 
A: 비대칭 서킷 브레이커 (CB 발동 시 롱만 차단, 숏은 지속 허용)
B: 완전 셧다운 (CB 발동 시 48시간 롱/숏 전면 차단)
"""
import asyncio
...
CB_ROE = -4.0
CB_COOLDOWN_MS = 48 * 3600 * 1000  # 48 hours
GLOBAL_MDD_LIMIT = -10.0
...
def simulate(data, btc1h_map, mode='B'):
    # mode 'A': Asymmetric (Short allowed during CB)
    # mode 'B': Symmetric (Total shutdown during CB)
    ...
    for idx, t in enumerate(all_ts):
        ...
        # Current Equity
        equity = sum(p['margin'] * (1 + (p['last_px'] - p['entry']) / p['entry'] * (LONG_LEVERAGE if p['dir'] == 1 else SHORT_LEVERAGE) * p['dir'])
        ...
        if weekly_mdd_pct <= GLOBAL_MDD_LIMIT:
            weekly_locked = True
        ...
        if daily_chg <= CB_ROE and not cb_tripped:
            cb_tripped = True
            cb_cooldown_until = t + CB_COOLDOWN_MS
        ...
        allow_long = not weekly_locked
        allow_short = not weekly_locked
        ...
        for sym, df in data.items():
            ...
            # Falling knife check (-10% in last 24h) roughly 96 candles of 15m
            if i >= 96:
            ...
            # Re-calculate equity for margin scaling
            equity = sum(p['margin'] * (1 + (p['last_px'] - p['entry']) / p['entry'] * (LONG_LEVERAGE if p['dir'] == 1 else SHORT_LEVERAGE) * p['dir'])
                         for p in positions.values()) + cash
...
async def main():
    ...
    try:
        ...
        for i in range(1, len(btc) - 1):
            ...
            btc_map[int(r['t'])] = (float(r['adx']), above200, float(r['dd']))
        ...
        print(f"\n백테스트 시작: 서킷 브레이커 충돌 해결 방안 비교 (Short 5x)\n")
        variants = [
            ("Option A (비대칭: 숏허용)", 'A'),
            ("Option B (대칭: 완전정지)", 'B')
        ]
        hdr = f"{'테스트옵션':24s} {'전체수익':>10s} {'E1횡보':>10s} {'E2랠리':>10s} {'E3최근폭락':>10s} {'숏손익':>10s} {'MDD%':>6s}"
        print(hdr)
        print("─" * len(hdr))
