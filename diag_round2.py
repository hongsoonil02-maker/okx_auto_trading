    ...
    for t in all_ts:
        ...
        for sym, df in data.items():
            ...
            if pos:
                ...
                if not exit_now and pos['max_pnl'] >= ATR_TRAIL_ARM:
                    atr = float(curr['atr'])
...
async def main():
    ...
    configs = [
        ("B0_baseline", {}),
        ("B1_atrStop1.5", {'atr_stop_mult': 1.5}),
        ("B2_aStp+noLo", {'atr_stop_mult': 1.5, 'no_low_adx': True}),
        ("B3_aStp+ban5", {'atr_stop_mult': 1.5, 'banned': BAN5}),
        ("B4_all3", {'atr_stop_mult': 1.5, 'no_low_adx': True, 'banned': BAN5}),
    ]
    ...
    # 진단: ATR스톱 베이스라인 거래 로그
    base_full = simulate(btc_map, data, {'atr_stop_mult': 1.5}, log_trades=True)
