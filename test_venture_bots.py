...
def simulate(df, ema_period=200, tight_mult=2.5, loose_mult=4.0, vol_mult=1.2, min_hold=3, max_dca=3, tp_thr=1.02):
    trades = []
...
async def main():
    ...
    print("="*80)
    print(" 🚀 Venture Bots (15m TF) Backtest - 10x Leverage")
    print("="*80)
    
    print("\n[Venture Alts] - 15m TF, EMA 200, TP 1.02")
    print(f"{'Symbol':<15} | {'DCA 3':<20} | {'DCA 8 (Current)':<20}")
    crypto_syms = ["DOGE/USDT:USDT", "PEPE/USDT:USDT", "WIF/USDT:USDT"]
    for sym in crypto_syms:
        try:
            ...
            tr_3 = simulate(df.copy(), ema_period=200, tp_thr=1.02, max_dca=3)
            tr_8 = simulate(df.copy(), ema_period=200, tp_thr=1.02, max_dca=8)
            
            s_3 = stats(tr_3); s_8 = stats(tr_8)
            f = lambda s: f"{s['ret']:>6.2f}% ({s['n']:>2d})"
            print(f"{sym:<15} | {f(s_3):<20} | {f(s_8):<20}")
        except Exception as e:
            print(f"{sym:<15} | ❌ Error: {str(e)}")
        await asyncio.sleep(0.5)
        
    print("\n[Stock Ventures] - 15m TF, EMA 200, TP 1.02")
    print(f"{'Symbol':<15} | {'DCA 3':<20} | {'DCA 8 (Current)':<20}")
    stock_syms = ["CRM/USDT:USDT", "OKTA/USDT:USDT", "TMF/USDT:USDT"]
