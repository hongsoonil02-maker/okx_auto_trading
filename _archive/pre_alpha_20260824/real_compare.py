...
def simulate_old(df, ema_period=50, max_dca=8, tp_thr=1.01):
    ...
    for i in range(250, len(df)):
        ...
        if short_pos:
            ...
            std = curr['st_d_t'] if is_pft else curr['st_d_l']
            close_s = std == 1 or curr['c'] > (curr['st_v_t'] if is_pft else curr['st_v_l'])
            if close_s:
        ...
        if not long_pos and not short_pos:
            if is_long_sig: long_pos = {'entry': curr['c'], 'size': 1.0/max_dca, 'entry_count': 1}
            elif is_short_sig: short_pos = {'entry': curr['c'], 'size': 1.0/max_dca, 'entry_count': 1}
    return trades
...
def simulate_new(df, ema_period=50, max_dca=12, tp_thr=1.01):
    ...
    df['st_d_l'], df['st_v_l'] = calc_supertrend(df, 10, 4.0)
    df['ema_target'] = df['c'].ewm(span=ema_period, adjust=False).mean()
    # Smooth slope to avoid noise
    df['ema_slope'] = df['ema_target'].diff(3)
    ...
    for i in range(250, len(df)):
        ...
        # Only enter if slope is clearly positive/negative
        ...
        if long_pos:
            ...
            # Maintain position if slope is strongly intact
        ...
        if short_pos:
            ...
            st_close_s = std == 1 or curr['c'] > (curr['st_v_t'] if is_pft else curr['st_v_l'])
        ...
        if not long_pos and not short_pos:
            if is_long_sig: long_pos = {'entry': curr['c'], 'size': 1.0/max_dca, 'entry_count': 1, 'first_i': i}
            elif is_short_sig: short_pos = {'entry': curr['c'], 'size': 1.0/max_dca, 'entry_count': 1, 'first_i': i}
    return trades

def calc_stats(trades):
    if not trades: return {'ret': 0, 'n': 0, 'wins': 0, 'wr': 0.0}
    pnls = [t['pnl'] for t in trades]
    ret = sum(pnls) * 100
    wins = len([p for p in pnls if p > 0])
    return {'ret': ret, 'n': len(trades), 'wins': wins, 'wr': wins/len(trades)*100}
...
async def main():
    ...
    for sym in syms:
        try:
            ohlcv = await ex.fetch_ohlcv(sym, '15m', limit=3000) # Increased limit to get more trades
            ...
            s_old = calc_stats(simulate_old(df.copy()))
            s_new = calc_stats(simulate_new(df.copy()))
