...
def simulate(df, initial_capital=10000, leverage=10):
    ...
    base_margin = initial_capital * 0.1
    ...
    last_hard_stop_time = pd.Timestamp("2000-01-01").tz_localize(timezone.utc)
    ...
    for i in range(1, len(df)):
        ...
        if (t_curr - last_hard_stop_time).total_seconds() < HARD_STOP_COOLDOWN_HOURS * 3600:
            is_in_cooldown = True
            
        # Long Logic
        if long_pos is None:
            if is_uptrend and stoch_oversold and not is_in_cooldown:
                long_pos = {
        ...
        if short_pos is None:
            if is_downtrend and stoch_overbought and not is_in_cooldown:
                short_pos = {
                    'entry': row['c'], 'margin': base_margin, 'size': (base_margin * leverage) / row['c'],
                    'lowest': row['c']
                }
...
async def run_backtest():
    ...
    for sym in symbols:
        try:
            ...
            df['t'] = pd.to_datetime(df['t'], unit='ms').dt.tz_localize(timezone.utc)
    ...
    print(f"\n=== NEW BACKTEST RESULTS (Last 3 Days with 12h Cooldown) ===")
    print(f"Total Theoretical PnL (Old Logic + Cooldown): {total_pnl:.2f} USDT")
    print(f"Total Trades: {total_trades}")
    print(f"Original Old Logic PnL: -518.42 USDT")
