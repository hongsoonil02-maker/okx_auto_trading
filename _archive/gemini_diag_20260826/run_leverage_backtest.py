...
def simulate(data, btc_map, flags, leverage=10):
    """
...
async def main():
    ...
    try:
        ...
        print(f"{'-'*80}")
        print(f"{'레버리지':10s} {'전체수익':>10s} {'거래수':>6s} {'승률':>5s} {'최대MDD':>10s} {'롱손익':>10s} {'숏손익':>10s}")
        for lev in [5, 10, 15]:
            # 베이스라인
            r = simulate(data, btc_map, dict(F), leverage=lev)
            print(f"  {lev}x (Base) {r['total']:+10.2f} {r['n']:6d}   -        -      {r['long_pnl']:+10.2f} {r['short_pnl']:+10.2f}")
            # 풀옵션
            r2 = simulate(data, btc_map, dict(F, conviction=True, trailing=True, bear_short=True), leverage=lev)
            print(f"  {lev}x (Full) {r2['total']:+10.2f} {r2['n']:6d}   -        -      {r2['long_pnl']:+10.2f} {r2['short_pnl']:+10.2f}")
            print("")
    finally:
        await ex.close()
