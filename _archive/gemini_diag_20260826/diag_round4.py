...
def simulate(btc_map, data, cfg, start_ts=None, end_ts=None, log_trades=False, tidx=None):
    ...
    def get_lev(sym):
        ...
        return cfg.get('lev_major', 15) if is_major(sym) else 5
    ...
    for t in all_ts:
        ...
        for sym, df in data.items():
            ...
            if pos:
                ...
                exit_now = False
                reason = ""
                amult = cfg.get('atr_stop_mult')
                if amult:
                    # [개선안] 가격 기준 ATR 스탑 (레버리지 무관 변동성 손절)
                    atr_e = pos.get('entry_atr', 0.0)
                    if atr_e > 0:
                        if pos['dir'] == 1 and px <= pos['entry'] - amult * atr_e:
                            exit_now, reason = True, "atrstop"
                        elif pos['dir'] == -1 and px >= pos['entry'] + amult * atr_e:
                            exit_now, reason = True, "atrstop"
                    # 안전망: 마진 하드스탑만 유지
                    if not exit_now and pnl_pct <= get_hard_stop(sym):
                        exit_now, reason = True, "hard"
                else:
                    # 소프트/하드 스탑 (마진 기준, 라이브 동일)
                    if pnl_pct <= get_hard_stop(sym):
                        exit_now, reason = True, "hard"
                    elif pnl_pct <= SOFT_STOP and not cfg.get('no_soft'):
                        exit_now, reason = True, "soft"
                # ATR 트레일링 (마진수익 도달 후)
                arm = cfg.get('trail_arm', ATR_TRAIL_ARM)
                tk = cfg.get('trail_k', ATR_TRAIL_K)
                if not exit_now and pos['max_pnl'] >= arm:
                    atr = float(curr['atr'])
                    if pos['dir'] == 1 and px < pos['extreme_px'] - tk * atr:
                        exit_now, reason = True, "atrtrail"
                    elif pos['dir'] == -1 and px > pos['extreme_px'] + tk * atr:
                        exit_now, reason = True, "atrtrail"
                ...
                if exit_now:
                    ...
                    del positions[sym]
                    if reason in ("hard", "soft", "atrstop"):
...
async def main():
    ...
    BAN5 = {"LIT/USDT:USDT", "CHIP/USDT:USDT", "BOME/USDT:USDT",
            "FIL/USDT:USDT", "LINK/USDT:USDT"}
    configs = [
        ("D0_C4", {'lev_major': 10, 'no_soft': True}),
        ("D1_D4+noLoADx", {'lev_major': 10, 'no_soft': True, 'no_low_adx': True}),
        ("D2_D1+ban5", {'lev_major': 10, 'no_soft': True, 'no_low_adx': True,
                        'banned': BAN5}),
        ("D3_D2+trail", {'lev_major': 10, 'no_soft': True, 'no_low_adx': True,
                         'banned': BAN5, 'trail_arm': 0.15, 'trail_k': 2.2}),
        ("D4_D1only", {'no_soft': True, 'no_low_adx': True}),
    ]
