...
pkill -9 -f 'okx_stock_strategy.py' >/dev/null 2>&1 || true
pkill -9 -f 'okx_venture_strategy.py' >/dev/null 2>&1 || true
pkill -9 -f 'okx_stock_venture_strategy.py' >/dev/null 2>&1 || true
pkill -9 -f 'daily_analyzer.py' >/dev/null 2>&1 || true

for port in 8000 8001 8003 8005 8013; do
...
systemctl --user start master_bot_orchestrator bot_c_okx_swap okx_major_strategy okx_stock_strategy okx_venture_strategy okx_stock_venture_strategy

sleep 2
echo "✅ Coinbot 프로세스 구동 완료!"
ss -tlnp | grep -E ':(8000|8001|8003|8013)' || true
