        ...
        try:
            ...
            if has_long:
                ...
                if is_pft or is_mean_rev:
                    self.logger.info(f"[{symbol}] [금강대제 벤처 로직] 롱 익절/본절 조건 충족 (Mean Reversion)")
                    ...
                    self.logger.info(f"[{symbol}] [금강대제 벤처 로직] 롱 DCA 물타기 조건 충족")
                    await self._send_signal(symbol, "buy_long", size_multiplier=1.0/self.MAX_DCA_ENTRIES, leverage=self.STRATEGY_LEVERAGE)
            else:
                if is_long_sig:
                    self.logger.info(f"[{symbol}] [금강대제 벤처 로직] 신규 롱 진입 (RSI 과매도 + BB 하단 이탈)")
                    await self._send_signal(symbol, "buy_long", size_multiplier=1.0/self.MAX_DCA_ENTRIES, leverage=self.STRATEGY_LEVERAGE)
