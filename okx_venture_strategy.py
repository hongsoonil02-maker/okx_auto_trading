...
# ── LOGGING ──
logger = logging.getLogger("OKX_Venture_Strategy")
logger.setLevel(logging.INFO)
fh = logging.FileHandler(os.path.join(BASE_DIR, "okx_venture_strategy.log"), encoding="utf-8")
...
class OKXVentureStrategyBrain:
    def __init__(self):
        self.session = None
        self.exchange = None
        self.auto_active_pos = {}  # {(symbol, side): {'size': float}}
        self.dca_state = {}  # {symbol: {'entry_count': 0, 'exit_count': 0, 'last_entry_t': 0, 'last_exit_t': 0}}
    ...
    # =====================================================================
    # [Auto Trading Logic (Supertrend + RSI)]
    # =====================================================================
        ...
        try:
            ...
            for s, t in tickers.items():
                if s in markets and markets[s].get('swap') and 'USDT' in s:
                    base_coin = s.split('-')[0]
                    if base_coin not in majors_and_stocks and not any(b in s for b in blacklist):
                        data.append({'symbol': s, 'vol': t.get('quoteVolume', 0)})
            
            df = pd.DataFrame(data).sort_values(by='vol', ascending=False)
            if df.empty: return []
            return df['symbol'].head(30).tolist()
        except Exception as e:
    ...
    async def check_auto_logic(self, symbol):
        try:
            ...
            st_d_tight, st_v_tight = calc_supertrend(df, 10, 2.0)
            ...
            if has_long and avg_price_long > 0:
                is_profit_target = curr['c'] > avg_price_long * 1.025
                st_v_long = curr['st_v_tight'] if is_profit_target else curr['st_v_loose']
                st_d_long = curr['st_d_tight'] if is_profit_target else curr['st_d_loose']
            ...
            if has_short and avg_price_short > 0:
                is_profit_target = curr['c'] < avg_price_short * 0.975
                st_v_short = curr['st_v_tight'] if is_profit_target else curr['st_v_loose']
                st_d_short = curr['st_d_tight'] if is_profit_target else curr['st_d_loose']
