...
- Geumgang Mirror Logic (Mean Reversion BB + DCA)
- 15m timeframe, max leverage scaling
"""
import asyncio
import sys
import pandas as pd
...
class OKXMajorStrategyBrain(BaseStrategyBrain):
    ...
    async def check_auto_logic(self, symbol):
        try:
            ...
            tp_pct = 0.005 # 0.5% 가격이동 (50x 기준 25% 수익)
            ...
            if has_long:
                ...
                if is_pft or is_mean_rev:
                    self.logger.info(f"[{symbol}] [금강대제 로직] 롱 익절/본절 조건 충족 (Mean Reversion)")
