...
"""
run_cb_tuning.py
MDD ~26% 타겟 고수익 튜닝 백테스트 (일일 서킷 브레이커 발동 기준 완화)
"""
import asyncio
...
def simulate(data, btc1h_map, cb_roe_limit, cooldown_hours=48):
    cash = INITIAL_EQUITY
    positions = {}
    trades = []
    fees = 0.0
    ...
    for idx, t in enumerate(all_ts):
        ...
        if weekly_mdd_pct <= -25.0:  # Relaxed weekly lock for this test
            weekly_locked = True
        ...
        if cb_roe_limit is not None and daily_chg <= cb_roe_limit and not cb_tripped:
            cb_tripped = True
            cb_cooldown_until = t + (cooldown_hours * 3600 * 1000)
        ...
        for sym, df in data.items():
            ...
            # Relaxed falling knife to allow trades
            if i >= 96:
                price_24h_ago = df.iloc[i-96]['c']
                if (px - price_24h_ago) / price_24h_ago <= -0.20:
                    allow_long = False
    ...
    return {
        'total': total, 'eras': era_pnls, 'mdd': mdd
    }
...
async def main():
    ...
    try:
        ...
        print(f"\n[MDD 26% 타겟] 서킷 브레이커 ROE 발동 한도 완화 테스트\n")
        limits = [
            ("CB -4% (현재)", -4.0, 48),
            ("CB -8% / 24h", -8.0, 24),
            ("CB -12% / 24h", -12.0, 24),
        ...
        hdr = f"{'서킷기준':18s} {'전체수익':>10s} {'E1횡보':>10s} {'E2랠리':>10s} {'E3최근폭락':>10s} {'MDD%':>6s}"
        print(hdr)
        print("─" * len(hdr))
        for name, roe, cd in limits:
            r = simulate(data, btc_map, cb_roe_limit=roe, cooldown_hours=cd)
            e = r['eras']
            print(f"  {name:16s} {r['total']:+10.0f} {e['E1 횡보(6/23~7/24)']:+10.0f} {e['E2 랠리(7/25~8/20)']:+10.0f} "
                  f"{e['E3 붕괴(8/21~8/29)']:+10.0f} {r['mdd']:5.1f}%")
    finally:
        await ex.close()
